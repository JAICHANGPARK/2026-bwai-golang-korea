---
title: Statistics And Probability Strand
type: synthesis
status: active
updated: 2026-04-10
source_docs:
  - docs/math-curriculum-research/korea.md
  - docs/math-agent-knowledge-plan.md
  - docs/math-concept-encyclopedia/korea.md
tags:
  - synthesis
  - statistics
  - probability
  - curriculum
  - strand
---

# Statistics And Probability Strand

## Summary

`확률·통계 계통`은 표와 그래프를 읽는 중등 데이터 문해력에서 시작해, 경우의 수, 조건부확률, 확률변수, 분포, 통계적 추정, 회귀로 이어지는 세로 축이다. 그래프에서는 `자료 읽기 -> 사건 세기 -> 숫자화 -> 분포화 -> 추정과 모델링`의 흐름이 보여야 한다.

## Key Points

- 시작 구간
  - [data-organization.md](./concept-cards/data-organization.md), [correlation-and-scatter-plots.md](./concept-cards/correlation-and-scatter-plots.md)
- 확률 입문
  - [counting-principles.md](./concept-cards/counting-principles.md), [conditional-probability.md](./concept-cards/conditional-probability.md)
- 분포 중심 구간
  - [random-variable.md](./concept-cards/random-variable.md), [probability-distribution.md](./concept-cards/probability-distribution.md), [normal-distribution.md](./concept-cards/normal-distribution.md)
- 추정과 모델링
  - [statistical-inference.md](./concept-cards/statistical-inference.md), [regression.md](./concept-cards/regression.md), [practical-statistics.md](./concept-cards/practical-statistics.md)
- 과목 허브
  - 한국은 [probability-and-statistics-course.md](./concept-cards/probability-and-statistics-course.md)가 중심이고, 이후 [practical-statistics.md](./concept-cards/practical-statistics.md), [mathematics-for-ai.md](./concept-cards/mathematics-for-ai.md)로 확장된다.
- 자주 보이는 가로 연결
  - `경우의 수 <-> 확률`
  - `조건부확률 <-> 독립`
  - `확률변수 <-> 확률분포`
  - `분포 <-> 통계적 추정`
  - `상관계수 <-> 회귀`

## Connections

- 학년 허브로는 [middle-1-hub.md](./middle-1-hub.md), [middle-2-hub.md](./middle-2-hub.md), [middle-3-hub.md](./middle-3-hub.md), [high-1-hub.md](./high-1-hub.md), [high-2-hub.md](./high-2-hub.md), [high-3-hub.md](./high-3-hub.md)를 함께 본다.
- 국가 비교는 [cross-country-statistics-and-data.md](./cross-country-statistics-and-data.md)와 연결된다.
- 평가 축은 [assessment-patterns.md](./assessment-patterns.md), [assessment-systems-comparison.md](./assessment-systems-comparison.md), [csat-2028.md](./csat-2028.md)로 이어진다.

## Open Questions

- `확률`을 중학교 표현에 맞춰 더 독립 허브로 뺄지 기준이 남아 있다.
- `실용 통계`와 `인공지능 수학`을 같은 데이터 응용 허브로 묶을지 후속 판단이 필요하다.

## Sources

- `docs/math-curriculum-research/korea.md`
- `docs/math-agent-knowledge-plan.md`
- `docs/math-concept-encyclopedia/korea.md`

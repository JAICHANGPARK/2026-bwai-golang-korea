---
title: 중국 8학년 2학기 수학
type: synthesis
status: active
updated: 2026-04-10
card_role: wrapper
source_docs:
  - docs/math-curriculum-research/china.md
  - docs/math-concept-encyclopedia/china.md
tags:
  - course
  - china
  - middle-school
---

# 중국 8학년 2학기 수학

## Summary

> [!info] Course Wrapper
> 이 카드는 개념 자체를 정의하는 페이지가 아니라, 과목이나 경로 안에서 개념들이 어떻게 묶여 배우는지를 안내하는 허브다.

중국 `8학년 2학기`는 이차근식, 피타고라스 정리, 평행사변형, 일차함수, 데이터 분석이 함께 놓인다. 대수 계산, 기하 성질, 함수 모델링이 동시에 중층화되는 학기다.

## Key Points

- 구성
  - 이차근식
  - 피타고라스 정리
  - 평행사변형
  - 일차함수
  - 데이터 분석
- 대표 개념
  - [square-roots-and-real-numbers.md](./square-roots-and-real-numbers.md)
  - [pythagorean-theorem.md](./pythagorean-theorem.md)
  - [linear-function.md](./linear-function.md)
  - [data-organization.md](./data-organization.md)
  - [correlation-and-scatter-plots.md](./correlation-and-scatter-plots.md)
- 교육과정 배치
  - 중국 중학 함수 위계에서 `일차함수`가 본격화되는 대표 학기다.

## Connections

- 선수 카드로는 [china-grade-8-semester-1.md](./china-grade-8-semester-1.md)가 있다.
- 다음 카드로는 [china-grade-9-semester-1.md](./china-grade-9-semester-1.md)가 이어진다.
- 국가 허브로는 [../china-curriculum-hub.md](../china-curriculum-hub.md)를 본다.

## Open Questions

- `평행사변형`을 [geometric-proof.md](./geometric-proof.md)와 별도 카드로 연결할지 후속 설계가 필요하다.

## Sources

- `docs/math-curriculum-research/china.md`
- `docs/math-concept-encyclopedia/china.md`

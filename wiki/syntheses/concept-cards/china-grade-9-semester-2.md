---
title: 중국 9학년 2학기 수학
type: synthesis
status: active
updated: 2026-04-10
card_role: wrapper
source_docs:
  - docs/math-curriculum-research/china.md
  - docs/math-concept-encyclopedia/china.md
tags:
  - course
  - china
  - middle-school
---

# 중국 9학년 2학기 수학

## Summary

> [!info] Course Wrapper
> 이 카드는 개념 자체를 정의하는 페이지가 아니라, 과목이나 경로 안에서 개념들이 어떻게 묶여 배우는지를 안내하는 허브다.

중국 `9학년 2학기`는 반비례함수, 닮음, 예각 삼각함수, 투영과 투상도를 묶어 중학교 기하-함수 축을 마무리한다. 고등학교 삼각함수와 공간 해석으로 넘어가는 바로 직전의 허브다.

## Key Points

- 구성
  - 반비례함수
  - 닮음
  - 예각 삼각함수
  - 투영과 투상도
- 대표 개념
  - [inverse-proportion.md](./inverse-proportion.md)
  - [similarity.md](./similarity.md)
  - [trigonometric-ratio.md](./trigonometric-ratio.md)
  - [spatial-coordinates.md](./spatial-coordinates.md)
- 교육과정 배치
  - 중국 중학의 마지막 함수·기하 브리지 학기다.

## Connections

- 선수 카드로는 [china-grade-9-semester-1.md](./china-grade-9-semester-1.md)가 있다.
- 다음 카드로는 [china-required-curriculum.md](./china-required-curriculum.md)가 이어진다.
- 국가 허브로는 [../china-curriculum-hub.md](../china-curriculum-hub.md)를 본다.

## Open Questions

- `투영과 투상도`를 [spatial-coordinates.md](./spatial-coordinates.md)와 별도 시각화 카드로 더 분리할지 기준이 필요하다.

## Sources

- `docs/math-curriculum-research/china.md`
- `docs/math-concept-encyclopedia/china.md`

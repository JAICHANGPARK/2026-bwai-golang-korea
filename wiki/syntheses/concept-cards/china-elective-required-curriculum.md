---
title: 중국 선택필수 과정 수학
type: synthesis
status: active
updated: 2026-04-10
card_role: wrapper
source_docs:
  - docs/math-curriculum-research/china.md
  - docs/math-concept-encyclopedia/china.md
tags:
  - course
  - china
  - high-school
---

# 중국 선택필수 과정 수학

## Summary

> [!info] Course Wrapper
> 이 카드는 개념 자체를 정의하는 페이지가 아니라, 과목이나 경로 안에서 개념들이 어떻게 묶여 배우는지를 안내하는 허브다.

중국 고교 `선택필수 과정`은 수열과 도함수, 공간벡터와 해석기하, 확률변수·정규분포·회귀, 프로젝트형 탐구를 포함하는 진학 중심 심화 경로다. 실제 가오카오 연결성이 가장 직접적으로 느껴지는 고교 심화 층이다.

## Key Points

- 구성
  - 수열과 도함수
  - 공간벡터와 해석기하
  - 확률변수·정규분포·회귀
  - 프로젝트형 수학 탐구
- 대표 개념
  - [sequences.md](./sequences.md)
  - [derivative.md](./derivative.md)
  - [vectors.md](./vectors.md)
  - [equations-of-geometric-figures.md](./equations-of-geometric-figures.md)
  - [random-variable.md](./random-variable.md)
  - [normal-distribution.md](./normal-distribution.md)
  - [regression.md](./regression.md)
  - [mathematical-modeling-and-inquiry.md](./mathematical-modeling-and-inquiry.md)
- 교육과정 배치
  - 전형적으로 고2 이후 진학 중심 경로의 핵심을 이룬다.

## Connections

- 선수 카드로는 [china-required-curriculum.md](./china-required-curriculum.md)가 있다.
- 다음 카드로는 [china-elective-concept-clusters.md](./china-elective-concept-clusters.md)가 이어진다.
- 국가 허브로는 [../china-curriculum-hub.md](../china-curriculum-hub.md)를 본다.

## Open Questions

- `수열과 도함수`를 중국 전용 wrapper로 별도 분리할지 검토가 필요하다.

## Sources

- `docs/math-curriculum-research/china.md`
- `docs/math-concept-encyclopedia/china.md`

...
- `Gemini CLI` 경로
  ...
  - 실제 `ADK` 앱으로 이어질 때는 `wiki enricher` prompt만으로 끝내지 말고, local `wiki lookup helper`나 `context builder`를 코드에 두는 편이 안정적이다.
- `Antigravity` 경로
  ...
  - 실제 `ADK` 앱으로 이어질 때는 `wiki/index.md`와 관련 page를 읽는 local helper를 starter kit 기준으로 연결하는 편이 안전하다.
